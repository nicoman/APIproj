...
def is_valid_dataset(platform):
    ...
    required the name and abbreviation of the platform.
...
def generate_plot(platforms, output_file):
    ...
    # x-axis and "values" the calue of each label on the y-axis:
    labels = []
    values = []
    for platform in platforms:
        ...
        # Skip prices higher than 2000 USD sumply because it would make the
        # output unusable.
        if price > 2000:
            continue
    ...
    # Let's define the idth of each bar and the size of the resulting graph.
    width = 0.3
...
def generate_csv(platforms, output_file):
    ...
    # we will habe to open first for writing. Otherwise we just assume that
...
def parse_args():
    ...
    parser.add_argument('--giantbomb-api-key', required=True,
                        help='API key provided by Giantbombo.com')
...
def main():
    ...
    print("Disclaimer: This script uses data provided by FRED, Federal"
            " Reserve Economic Data, from the Federal Reserve Banco of St. Louis"
